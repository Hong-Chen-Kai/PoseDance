...
const state = {
  ...
  latestUserLandmarks2: null,

  many: {
    detectorKind: "mp", // 'mp' | 'coco'
    mpDetector: null,
    mpVision: null,
    cocoModel: null,
    bboxSmooth: { enabled: true, alpha: 0.5 },
...
function initDomRefs() {
  ...
  els.detectorSelect = $("detectorSelect");
  els.bboxSmoothToggle = $("bboxSmoothToggle");
...
function bboxFromCoco(pred) {
  ...
  if (![x, y, w, h].every((n) => typeof n === "number" && Number.isFinite(n)))
    return null;
  return { x, y, w, h, score: typeof pred.score === "number" ? pred.score : 0 };
}

function bboxFromMp(det) {
  ...
  if (![x, y, w, h].every((n) => typeof n === "number" && Number.isFinite(n)))
    return null;
  return { x, y, w, h, score: typeof det?.categories?.[0]?.score === "number" ? det.categories[0].score : 0 };
}
...
function centerDistNorm(a, b) {
  ...
  const dx = acx - bcx;
  const dy = acy - bcy;
  const denom = Math.max(1, Math.sqrt((a.w * a.h + b.w * b.h) / 2));
  return Math.sqrt(dx * dx + dy * dy) / denom;
}
...
function pickTop2People(bbs) {
  const arr = (Array.isArray(bbs) ? bbs : [])
    .filter((b) => b && b.w > 4 && b.h > 4)
...
function matchTracks(prevA, prevB, detA, detB) {
  // return [newA, newB] matched to keep identity as stable as possible
  if (!prevA && !prevB) return [detA || null, detB || null];
  const cands = [detA, detB].filter(Boolean);
  if (cands.length === 0) return [null, null];
  if (cands.length === 1) {
    const d = cands[0];
    const scoreA = prevA ? iou(prevA, d) - 0.2 * centerDistNorm(prevA, d) : -1;
    const scoreB = prevB ? iou(prevB, d) - 0.2 * centerDistNorm(prevB, d) : -1;
    if (scoreA >= scoreB) return [d, null];
    return [null, d];
  }
  const d1 = detA;
  const d2 = detB;
  const s11 = prevA ? iou(prevA, d1) - 0.2 * centerDistNorm(prevA, d1) : 0;
  const s12 = prevA ? iou(prevA, d2) - 0.2 * centerDistNorm(prevA, d2) : 0;
  const s21 = prevB ? iou(prevB, d1) - 0.2 * centerDistNorm(prevB, d1) : 0;
  const s22 = prevB ? iou(prevB, d2) - 0.2 * centerDistNorm(prevB, d2) : 0;
  const keep = s11 + s22;
  const swap = s12 + s21;
  if (swap > keep) return [d2, d1];
  return [d1, d2];
}

function expandAndClampBbox(b, vw, vh, padRatio = 0.15) {
  const padX = b.w * padRatio;
  const padY = b.h * padRatio;
  let x = b.x - padX;
  let y = b.y - padY;
  let w = b.w + padX * 2;
  let h = b.h + padY * 2;
  ...
  w = clamp(w, 2, vw - x);
  h = clamp(h, 2, vh - y);
  return { x, y, w, h, score: b.score };
}

function cropToCanvas(video, bbox, targetShortEdge = 512, reuseCanvas) {
  const vw = video.videoWidth || 0;
  const vh = video.videoHeight || 0;
  if (!vw || !vh || !bbox) return null;
  const b = expandAndClampBbox(bbox, vw, vh, 0.18);
  const short = Math.min(b.w, b.h);
  const s = targetShortEdge / Math.max(1, short);
  const tw = Math.max(1, Math.round(b.w * s));
  const th = Math.max(1, Math.round(b.h * s));
  ...
  ctx.drawImage(video, b.x, b.y, b.w, b.h, 0, 0, tw, th);
  return { canvas: c, bbox: b, scale: s };
}

function mapLandmarksToFullNorm(lm, bbox, vw, vh) {
  ...
  for (let i = 0; i < 33; i += 1) {
    ...
    const x = (bbox.x + (p.x || 0) * bbox.w) / Math.max(1, vw);
    const y = (bbox.y + (p.y || 0) * bbox.h) / Math.max(1, vh);
    out[i] = {
      x: clamp01(x),
      y: clamp01(y),
...
async function ensureMpDetector() {
  if (state.many.mpDetector) return state.many.mpDetector;
  const { FilesetResolver, ObjectDetector } = await import("@mediapipe/tasks-vision");
  const vision = await FilesetResolver.forVisionTasks(
    "https://cdn.jsdelivr.net/npm/@mediapipe/tasks-vision@latest/wasm",
  );
  state.many.mpVision = vision;
  state.many.mpDetector = await ObjectDetector.createFromOptions(vision, {
    baseOptions: {
      modelAssetPath:
  ...
  console.log("✅ MP ObjectDetector 初始化完成");
  return state.many.mpDetector;
}

async function ensureCocoDetector() {
  if (state.many.cocoModel) return state.many.cocoModel;
  const tf = await import("https://esm.sh/@tensorflow/tfjs");
  // backend 選 webgl，失敗就退回 cpu
  try {
  ...
  await tf.ready();
  const cocoSsd = await import("https://esm.sh/@tensorflow-models/coco-ssd");
  state.many.cocoModel = await cocoSsd.load();
  console.log("✅ COCO-SSD 初始化完成");
  return state.many.cocoModel;
}

async function detectPeople(video, tsMs) {
  const kind = state.many.detectorKind;
  if (kind === "coco") {
    const model = await ensureCocoDetector();
  ...
  const det = await ensureMpDetector();
  const res = det.detectForVideo(video, tsMs);
  return (res?.detections || []).map(bboxFromMp).filter(Boolean);
}

async function initPose() {
  if (!els.startCameraButton) return;

  if (els.detectorSelect) {
    state.many.detectorKind = els.detectorSelect.value === "coco" ? "coco" : "mp";
    els.detectorSelect.addEventListener("change", () => {
      state.many.detectorKind = els.detectorSelect.value === "coco" ? "coco" : "mp";
      console.log("🔁 切換偵測器:", state.many.detectorKind);
    });
  }
  if (els.bboxSmoothToggle) {
    state.many.bboxSmooth.enabled = !!els.bboxSmoothToggle.checked;
    els.bboxSmoothToggle.addEventListener("change", () => {
      state.many.bboxSmooth.enabled = !!els.bboxSmoothToggle.checked;
      console.log("🟦 BBox 平滑:", state.many.bboxSmooth.enabled ? "ON" : "OFF");
    });
  }
  ...
  els.startCameraButton.addEventListener("click", async () => {
    if (state.cameraRunning) {
      ...
      state.latestUserLandmarks = null;
      state.latestUserLandmarks2 = null;
      drawUserOverlay();
      return;
    }
    ...
    try {
      ...
      const processFrames = async () => {
        ...
        try {
          ...
          const vw = frame.videoWidth || 0;
          const vh = frame.videoHeight || 0;
          if (!vw || !vh) return;

          // detect people (stress: every frame)
          const td0 = performance.now();
          const bbsAll = await detectPeople(frame, currentTimestampMs);
          const td1 = performance.now();

          const picked = pickTop2People(bbsAll);
          const detA = picked[0] || null;
          const detB = picked[1] || null;

          // match to tracks
          let [mA, mB] = matchTracks(state.many.tracks.a, state.many.tracks.b, detA, detB);

          // smooth
          if (state.many.bboxSmooth.enabled) {
            mA = mA ? smoothBbox(state.many.tracks.a, mA, state.many.bboxSmooth.alpha) : null;
            mB = mB ? smoothBbox(state.many.tracks.b, mB, state.many.bboxSmooth.alpha) : null;
          }
          state.many.tracks.a = mA;
          state.many.tracks.b = mB;
          ...
          state.latestUserLandmarks = lmA;
          state.latestUserLandmarks2 = lmB;

          // stats (console)
          const t1 = performance.now();
          const st = state.many.stats;
          st.frames += 1;
          st.detectMs += td1 - td0;
          st.pose1Ms += p1t1 - p1t0;
          st.pose2Ms += p2t1 - p2t0;
          st.e2eMs += t1 - t0;
          if (pendingFrame) st.dropped += 1;
          if (t1 - st.lastLogAt > 1000) {
            const n = Math.max(1, st.frames);
            console.log(
              `[ManyPeople] fps~${Math.round((n * 1000) / Math.max(1, t1 - st.lastLogAt))} ` +
                `detect=${(st.detectMs / n).toFixed(1)}ms ` +
                `pose1=${(st.pose1Ms / n).toFixed(1)}ms ` +
                `pose2=${(st.pose2Ms / n).toFixed(1)}ms ` +
                `e2e=${(st.e2eMs / n).toFixed(1)}ms ` +
                `dropped=${st.dropped} ` +
                `det=${state.many.detectorKind} smooth=${state.many.bboxSmooth.enabled ? "on" : "off"}`,
            );
            st.lastLogAt = t1;
            st.frames = 0;
            st.detectMs = 0;
...
function drawMode2Overlay(tScore) {
  ...
  if (state.latestUserLandmarks2) {
    const userColor2 = "rgba(168,85,247,0.95)";
    drawPoseConnections(ctx, state.latestUserLandmarks2, getLmXYV, rectUser, () => userColor2, 3);
    drawPosePoints(ctx, state.latestUserLandmarks2, getLmXYV, rectUser, userColor2, 3.5);
  }
...
function updateUiLoop() {
  ...
  if (els.overlayCanvas) {
    ...
    if (ctx) {
      ...
      const purpleColor = "rgba(168,85,247,0.95)";
      ...
      // second user (many-people test)
      const userLm2 = state.latestUserLandmarks2;
      if (userLm2) {
        drawPoseConnections(ctx, userLm2, getLmXYV, stageRect, () => purpleColor, 3);
        drawPosePoints(ctx, userLm2, getLmXYV, stageRect, purpleColor, 3.5);
      }
