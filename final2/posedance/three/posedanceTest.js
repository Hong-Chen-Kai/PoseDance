...
const PROCEDURAL_IMPORT_BUILD = "planted-feet-5";
...
/** 使用者骨架（黑底上較清楚） */
const USER_SKELETON_COLOR = "rgba(255, 149, 0, 0.98)";
const USER_SKELETON_COLOR_GOOD = "rgba(255, 193, 77, 1)";
...
function stopCameraIfRunning() {
  ...
  state.latestUserWorldLandmarks = null;
  drawUserOverlay();
}
  ...
  els.startCameraButton.addEventListener("click", async () => {
    ...
    try {
      ...
      PoseModel.setCallback((result) => {
        ...
        state.latestUserWorldLandmarks = result.worldLandmarks || null;
      });
...
function drawMode2Overlay(tScore) {
  ...
  const userColor = USER_SKELETON_COLOR;
